...
GUIDE_EFFICIENCIES = [1.0, 0.66, 0.33]
...
def main():
    ...
    try:
        ...
        TARGET_GENES_TO_KNOCKDOWN = [g for g in TARGET_GENES_TO_KNOCKDOWN if g >= 20]
        
        if not TARGET_GENES_TO_KNOCKDOWN:
            print(f"Error: No target genes >= 20 found in {INPUT_TARGETS_FILE}.")
            sys.exit(1)
            
        print(f"Found {len(TARGET_GENES_TO_KNOCKDOWN)} target genes (ID 20d+) to intervene on.")
